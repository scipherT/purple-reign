...
#define SET_PIN_52 {REG_PIO_PIN_52_SODR = PIN_52_BITMASK;}
#define CLR_PIN_52 {REG_PIO_PIN_52_CODR = PIN_52_BITMASK;}
#define SET_PIN_53 {REG_PIO_PIN_53_SODR = PIN_53_BITMASK;}
#define CLR_PIN_53 {REG_PIO_PIN_53_CODR = PIN_53_BITMASK;}
...
void togglePinB() {
  ...
  if (toggle) SET_PIN_52 // digitalWrite(52,HIGH);
  else CLR_PIN_52 // digitalWrite(52,LOW);
  toggle = !toggle;
}
...
void togglePinA() {
  static bool toggle = false;
  if (toggle) SET_PIN_53 // digitalWrite(53,HIGH);
  else CLR_PIN_53 // digitalWrite(53,LOW);
  toggle = !toggle;
}
...
unsigned long _nextTickMajor = 0; // time stamp in microseconds indicating next absolute time to perform a Major "tick"
unsigned long _nextTickMinor = 0; // time stamp in microseconds indicating next absolute time to perform a Minor "tick"
unsigned long _nextTickADC = 0; // time stamp in microseconds indicating next absolute time to perform an ADC "tick"
...
uint32_t _rowPortBitPattern; // Remember the row port bit pattern from previous lap in the current scan loop (or, if current lap is the first; from the last lap in the previous scan loop)
...
// #define ROW_PORT_DISABLE_OUTPUT_DATA_REG FAKE_REG_PIOD_SODR // Disabling is done by setting port pin to HIGH (*Set* Output Data Register; SODR)
// #define ROW_PORT_ENABLE_OUTPUT_DATA_REG FAKE_REG_PIOD_CODR // Enabling is done by clearing port pin to LOW (*Clear* Output Data Register; CODR)
// uint32_t FAKE_REG_PIOD_SODR, FAKE_REG_PIOD_CODR;
...
enum mapType_t {LIN_STD, LIN_1, LIN_2, LOG_STD, LOG_1, LOG_2, EXP_8};
...
switchArray_t switchArray = {  // switch array for the matrix representing the lower half of the keybed
  {
...
switchArray_t switchArrayFake = {  // switch array for the matrix representing the lower half of the keybed, simulating the real switches
  {
    { {HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH},{HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH} },
    { {HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH},{HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH} },
    { {HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH},{HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH,HIGH} },
...
void midiLogSwitchChange(int con, int row, int mkbk, int col, int keySwitch, int prevKeySwitch) {
  ...
  if (keySwitch != prevKeySwitch) {
    if (keySwitch == LOW) { // key switch is pressed
      if (mkbk == MK) {
        mynoteon(a, 127, 10);
      } 
      else { // mkbk = BK
    ...
    else { // keySwitch == HIGH; key switch is released
      if (mkbk == MK) {
        mynoteoff(a, 127, 10);
      } 
      else { // mkbk = BK
...
enum LOG_ENTRY_TYPES {TickOverrunType, SwitchStateType};
...
void addEntryToLog(int time, int entryType, uint64_t value) {
  if(logArrayIx<logArraySize) {
    ...
    logArrayIx++;
    if(logArrayIx==logArraySize) dumpLog();
  }
}
...
void setup() {
    ...
    { // Configure ADC
      ...
      adcValPrevCh2 = adc_get_channel_value(ADC, ADC_CHANNEL_2); // Connected to general purposse controller 1
      adcValPrevCh3 = adc_get_channel_value(ADC, ADC_CHANNEL_3); // Connected to general purposse controller 2
      adcValPrevCh4 = adc_get_channel_value(ADC, ADC_CHANNEL_4); // Connected to general purposse controller 3
      adcValPrevCh5 = adc_get_channel_value(ADC, ADC_CHANNEL_5); // Connected to general purposse controller 4
    }

    // delayMicroseconds(10);  // Delay needed to give time for row pin signal to settle for very first read
...
void loop() {
    ...
    if (microSeconds > _nextTickMajor) {
      // Handle missed/not_missed ticks
      if (microSeconds > _nextTickMajor + _tickDeltaMajor) { //missed one or more ticks!
        #ifdef LOG_MISSED_TICKS
          logMissedTicks(microSeconds, (microSeconds - _nextTickMajor) / _tickDeltaMajor);
        #endif  
        _nextTickMajor = microSeconds - (microSeconds % _tickDeltaMajor) + _tickDeltaMajor; // FF to closest future _nextTickMajor
        // TODO (OPTIMIZE): This code should probably not happen often, if at all. Still... Use tick intervals with size 2^n. This means "Fast Forward" can be done easier, e.g. by setting the current time "tick fraction" bits to zero and do +1 on "tick integer" . E.g.;
        // * tick_size is 512 us (= 2^9 us)
        // * Assume now that current_time is > next_tick + tick_size (= missed one or more ticks)
        // * [CODE] if (current_time > next_tick + tick_size )  { next_tick = (time && FFFFFE00) + 1;}
        // * The above is correct *iff* the missed tick does not happen so that current time occurs exactly on an "integer" tick time, i.e. the "fraction" tick is zero. The one tick too much will be skipped. Otoh, skipping one tick too much might be OK once in a while.
      } else { // Didn't miss any tick(s), just advance next tick
        _nextTickMajor += _tickDeltaMajor;
      }
      ...
      for (int rowMkbk=0; rowMkbk<(numRows*numSwitches); rowMkbk++) {  
        ...
        // Scan/read the column pins
        // for (int con = 0; con < numConnectors; con++) {
        //   for (int col=0; col < numCols; col++) {
        //     #ifdef SIMULATE_KEYSWITCHES 
        //       colKeySwitch[col]=mySwitchRead(row,mkbk,col); // read the current switch state from simulated (fake) switch array
        //     #else
        //       colKeySwitch[con][col]=digitalRead(colPinList[con][col]); // read the current switch state from pin
        //     #endif
        //   }
        // }
        ...
        for (int con = 0; con < numConnectors; con++) {
          for (int col = 0; col < numCols; col++) {
            ...
            if (con == 0) keySwitch = 1 - keySwitch; // negate keySwitch for connector 0 , assuming 74HC14 is in series with conenctor 0 output. (74HC14 vs 74hc9015 workaround)
              ...
              if (switchMuteTimer[con][row][mkbk][col]>0) switchMuteTimer[con][row][mkbk][col]--; // decrement switchMuteTimer until reaches zero
              if (mkbk == MK) { // Velocity: Only act on MK switch scan, to avoid double stopwatch increments per key (NOTE! this does not mean that velocity stopwatch is only handled when MK switch is closed, just that it is handled only once per key)
                int stopwatch = keyVelocityStopwatch[con][row][col];
                if (stopwatch > 0 && stopwatch < velocityStopWatchMaxValue) keyVelocityStopwatch[con][row][col]++; // increment keyVelocityStopwatch within limits, 0 = not started; 1 = min, 1000 = max
              }
            #endif
            ...
            if (keySwitch != prevKeySwitch && switchMuteTimer[con][row][mkbk][col] == 0) { // if there is a new value from this scan, compared to previous scan AND switch is not muted
    ...
    if (microSeconds >= _nextTickADC) {
      _nextTickADC = microSeconds - (microSeconds % _tickDeltaADC) + _tickDeltaADC; // FF to closest future _nextTickADC

      // handle ADC channel 0 (pitch bend)
      int adcValCh0 = adc_get_channel_value(ADC, ADC_CHANNEL_0); // Connected to pitch bend
      if (adcValCh0 != adcValPrevCh0) enqueuePitchBend(adcValCh0, 1);
      ...
      if (adcValCh1 != adcValPrevCh1) enqueueCC(ccNumModulation, adcValCh1, 1);
      ...
      if (adcValCh2 != adcValPrevCh2) enqueueCC(ccNumGeneralPurpose1, adcValCh2, 1);
      ...
      if (adcValCh3 != adcValPrevCh3) enqueueCC(ccNumGeneralPurpose2, adcValCh3, 1);
      ...
      if (adcValCh4 != adcValPrevCh4) enqueueCC(ccNumGeneralPurpose3, adcValCh4, 1);
      ...
      if (adcValCh5 != adcValPrevCh5) enqueueCC(ccNumGeneralPurpose4, adcValCh5, 1);
    ...
    if (microSeconds >= _nextTickMinor) {
      // togglePinB();
      _nextTickMinor = microSeconds - (microSeconds % _tickDeltaMinor) + _tickDeltaMinor; // FF to closest future _nextTickMinor
